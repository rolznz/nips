...
*This is the original flow for initiating a NWC connection, however is less secure as potentially leaks the secret of the **client** during the initial connection process. Please see the client-created secret flows below as an alternative.*
...
- `identity` Optional. The hex-encoded identity `pubkey` of the **client** that the **wallet service** is connecting to. This is used for the wallet to display information about the **client** to the user.
...
- `request_methods` Optional. The url encoded, space separated list of request types that you need permission for: `pay_invoice` (default), `get_balance` (see NIP47). For example: `..&request_methods=pay_invoice%20get_balance`
...
- `isolated` Optional. The makes an isolated app connection with its own balance and only access to its own transaction list. e.g. `&isolated=true`. If using this option, you should not pass any custom request methods or notification types, nor set a budget or expiry.
...
#### Communicating the wallet pubkey and relay to the client

The wallet service MUST dispatch an event and post a message to the window and window opener (if present) so that the **client** can discover the pubkey and relay url of the **wallet service**.
...
If a wallet service is not accessible via HTTP, the **client** MAY generate an authorization URI with protocol `nostr+walletauth://` and base path its hex-encoded `pubkey` with the following query string parameters:

- `relay` Required. URL of the relay where the **client** intends to communicate with the **wallet service**.
...
If the **user** approves the request, a new connection should be created for **client**'s public key (specified in the base path of the authorization URI). Once the connection is created, the NIP-47 info event MUST be broadcasted to the relay specified in the authorization URI, and the info event MUST include a `p` tag containing the public key of the **client** this info is for, so that the **client** can discover the public key of the **wallet service**, and MAY contain a relay url, specifying the preferred relay of the **wallet service**. The info event MAY also include a `lud16` tag containing the lightning address of the **user**.

The **client** then may subscribe to NIP-47 info event (kind 13194) filtered by setting the `p` tag to the pubkey of the **client**. The client will then know the public key of the **wallet service** (as the publisher of the info event).
